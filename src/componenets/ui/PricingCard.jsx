...
const PricingCard = ({title,price,buttonName,planFeatures}) => {
  return (
    <Card className={`rounded-[17px] py-14  ${price === "Custom" ? "bg-purpleCardBackground":""}  px-8 my-4`}>
        ...
        <button className={`rounded-[6px]  text-[14px] bg-transparent w-full my-8 p-4 py-5  border-[1.8px] border-[#292a2d83] ${price.toLowerCase() === "free" ? "gradientBtn text-black" : "text-white"} `}>{buttonName}</button>
        <ul>
