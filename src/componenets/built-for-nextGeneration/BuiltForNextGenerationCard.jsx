...
const BuiltForNextGenerationCard = ({id,onClick,selectedTabId,className, img1Src,img2Src, title, desc }) => {
  ...
  return (
    <Card  onClick={onClick} className={`cursor-pointer ${id === selectedTabId ? "border-[#7C65D8] border-[1px] " :""} transition duration-500 hover:border-[1px] h-full py-6 px-4 w-full ${id === 4 ? "hidden md:block": ""} md:p-6 md:py-8 ${className} `}>
